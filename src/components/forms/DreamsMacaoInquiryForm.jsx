...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbyZ6nIVr5yzL5l3wOG5zG0gn4e-q1L0qI-BbLvxiYzT2HXxzAeYtkbuwY8muztPMLM69A/exec"

const defaultContent = {
  title: "Dreams Macao Inquiry Form",
  ctaText: "Get My Custom Wedding Proposal",
}

export default function DreamsMacaoInquiryForm({ content = defaultContent }) {
  ...
  const validateForm = () => {
    ...
    else if (!/^\+?[0-9\s()-]{7,20}$/.test(formData.phone.trim())) nextErrors.phone = "Enter a valid phone number."
    ...
    if (!formData.eventsCount) nextErrors.eventsCount = "Number of events is required."
    else if (Number(formData.eventsCount) < 1) nextErrors.eventsCount = "Events count must be at least 1."
    ...
    >
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input id="guestCount" name="guestCount" type="number" min="1" placeholder="e.g. 150" value={formData.guestCount} onChange={handleChange} />
          {errors.guestCount && <p className="text-sm text-destructive">{errors.guestCount}</p>}
        </div>
        <div className="space-y-2">
          <label htmlFor="eventsCount" className="text-sm font-medium text-foreground">Number of Events</label>
          <Input id="eventsCount" name="eventsCount" type="number" min="1" placeholder="e.g. 4" value={formData.eventsCount} onChange={handleChange} />
          {errors.eventsCount && <p className="text-sm text-destructive">{errors.eventsCount}</p>}
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input id="totalBudget" name="totalBudget" placeholder="e.g. 40000 USD" value={formData.totalBudget} onChange={handleChange} />
        ...
        <div className="space-y-2">
          ...
          <Input id="country" name="country" placeholder="Enter your country" value={formData.country} onChange={handleChange} />
      ...
      <div className="space-y-2">
        ...
        <Textarea id="vision" name="vision" placeholder="Tell us about your wedding vision" className="min-h-[120px]" value={formData.vision} onChange={handleChange} />
